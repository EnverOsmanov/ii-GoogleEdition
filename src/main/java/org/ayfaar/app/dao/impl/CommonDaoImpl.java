...
public class CommonDaoImpl implements CommonDao {
    ...
    protected <E> List<E> list(Criteria criteria) {
        return new ArrayList<>(new LinkedHashSet<>(criteria.list())); // privent duplications
    }
...
//                + " order by SUBSTRING(number FROM SUBSTRING_INDEX(number, \".\", 0))";
        ...
        List<Object[]> list = sessionFactory.getCurrentSession().createSQLQuery(
                ...
                .setMaxResults(limit)
                .list();
        List<Content> contents = list.stream().map(o -> new Content((String) o[0], (String) o[1], (String) o[2])).collect(Collectors.toList());
        return contents;
    }
...
//                + " order by SUBSTRING(number FROM SUBSTRING_INDEX(number, \".\", 0))";
        ...
        List<Object[]> list = sessionFactory.getCurrentSession().createSQLQuery(
                ...
                .list();
        return list.stream()
                   .map(o -> new Content((String) o[0], (String) o[1], (String) o[2]))
