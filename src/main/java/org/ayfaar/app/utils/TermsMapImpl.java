...
public class TermsMapImpl implements TermsMap {
    ...
    public class TermProviderImpl implements TermProvider {
        ...
        public List<String> getMorphs() {
            List<String> morphs = aliasesMap.entrySet().stream().filter(map -> map.getValue().getUri().equals(getUri())).map(Map.Entry<String, TermProvider>::getKey).collect(Collectors.toList());
            return morphs;
        }
    ...
    private List<TermProvider> getListProviders(byte type, String name) {
        ...
        return links.entrySet().stream()
                               .filter(link -> link.getValue().getType() == type &&
                                               link.getValue().getMainTerm().getName().equals(name))
                               .map(link -> aliasesMap.get(getValueFromUri(Term.class, link.getKey().toLowerCase())))
                               .collect(Collectors.toList());
    }

    @Deprecated // for old version and mediawiki sync support
    public List<Term> findTermsInside(String content) {
        Set<Term> contains = new HashSet<>();
        ...
        List<Term> sorted = new ArrayList<>(contains);
